...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        if not settings.DEBUG: raise CommandError("Demo data is only allowed in an explicit DEBUG development environment.")
        ...
        user,user_created=User.objects.get_or_create(email=opts["email"],defaults={"first_name":"Ahmed","last_name":"Kadiwala","phone":"1999000111","department":"admin"})
        ...
        customers=[Company.objects.create(tenant=tenant,name=n,address="Pune, Maharashtra") for n in ["Apex Engineering","Northstar Fabrication","Vertex Industrial","Cobalt Systems","Sterling Works"]]
        suppliers=[m.Supplier.objects.create(**base,name=n,contact_name=c,email=f"supplier{i}@example.test",phone=f"190000000{i}",payment_terms=30) for i,(n,c) in enumerate([("Pioneer Steel Co.","Rohan Shah"),("Metro Components","Priya Desai"),("Precision Supply","Vikram Patil")])]
        items=[]
        ...
        now=timezone.localdate();start=now.replace(day=1)
        employees=[]
        ...
        def document(kind,customer=None,supplier=None,business_date=None,amount=28000,status_post=True):
            ...
            calc=calculate_line({"quantity":1,"rate":amount,"tax_rate":0})
        ...
        for offset in range(5,-1,-1):
            ...
            for i in range(3):
                inv=document("invoice",customers[(i+offset)%5],business_date=month+timedelta(days=i*5+2),amount=92000+(5-offset)*13000+i*18000)
        ...
        orders=[]
        for i in range(6):
            order=document("sales_order",customers[i%5],amount=68000+i*19500,business_date=now-timedelta(days=i*2));order.due_date=now+timedelta(days=i*3-2);order.save();orders.append(order)
        for i,name in enumerate(["Warehouse racking · Phase 2","Precision machine enclosures","Conveyor support frames","Modular storage installation","Assembly line workstations"]):
            job=m.Job.objects.create(**base,name=name,number=svc.number(tenant,"job"),source_order=orders[i],customer=customers[i%5],quantity=50,due_date=now+timedelta(days=i*2-3),priority="high" if i==0 else "normal",status="in_progress",owner=user,instructions="Follow the approved customer drawing. Check dimensions before packing.")
        ...
        for customer in customers[:3]:document("quotation",customer,amount=115000,status_post=False)
        ...
        m.PayrollRun.objects.create(**base,name=f"{now:%B %Y} · Monthly payroll",month=start)
        self.stdout.write(self.style.SUCCESS(f"ERP demo created. tenant_id={tenant.pk}; login={user.email}. Synthetic records only; existing CRM tenant untouched."))
